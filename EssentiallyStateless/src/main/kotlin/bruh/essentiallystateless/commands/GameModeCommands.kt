...
class GameModeCommands(
    ...
    suspend fun gamemode(
        ...
        if (gameMode == null) {
            actor.sender().sendMessage(translations.getComponent(CommandMessages.INVALID_GAMEMODE) {
                unparsed("mode", mode)
            })
            return
        }
        ...
        if (targetPlayer == actor.sender()) {
            actor.sender().sendMessage(translations.getComponent(CommandMessages.GAMEMODE_SET_SELF) {
                unparsed("mode", gameMode.name.lowercase())
            })
        } else {
            actor.sender().sendMessage(translations.getComponent(CommandMessages.GAMEMODE_SET_OTHER) {
                unparsed("player", targetPlayer.name)
                unparsed("mode", gameMode.name.lowercase())
            })
            targetPlayer.sendMessage(translations.getComponent(CommandMessages.GAMEMODE_SET_BY_OTHER) {
                unparsed("mode", gameMode.name.lowercase())
    ...
    @CommandPermission("essentiallystateless.gamemode")
    suspend fun creative(actor: BukkitCommandActor, @Optional @SuggestOnlinePlayer target: String?) {
        gamemode(actor, "creative", target)
    }

    @Command("gms", "survival")
    @CommandPermission("essentiallystateless.gamemode")
    suspend fun survival(actor: BukkitCommandActor, @Optional @SuggestOnlinePlayer target: String?) {
        gamemode(actor, "survival", target)
    }

    @Command("gma", "adventure")
    @CommandPermission("essentiallystateless.gamemode")
    suspend fun adventure(actor: BukkitCommandActor, @Optional @SuggestOnlinePlayer target: String?) {
        gamemode(actor, "adventure", target)
    }

    @Command("gmsp", "spectator")
    @CommandPermission("essentiallystateless.gamemode")
    suspend fun spectator(actor: BukkitCommandActor, @Optional @SuggestOnlinePlayer target: String?) {
