...
class TeleportCommands(
    ...
    fun tp(
        ...
        if (playerToMove == actor.sender()) {
            actor.sender().sendMessage(translations.getComponentSync(CommandMessages.TP_SUCCESS) {
                unparsed("target", target.name)
            })
        } else {
            actor.sender().sendMessage(translations.getComponentSync(CommandMessages.TP_OTHER) {
    ...
    fun tphere(actor: BukkitCommandActor, @SuggestOnlinePlayer targetName: String) {
        if (actor.sender() !is Player) {
            actor.sender().sendMessage(translations.getComponentSync(CommandMessages.PLAYER_ONLY))
            return
        }
        ...
        actor.sender().sendMessage(translations.getComponentSync(CommandMessages.TPHERE_SUCCESS) {
    ...
    fun tpall(actor: BukkitCommandActor, @Optional @SuggestOnlinePlayer targetName: String?) {
        ...
        actor.sender().sendMessage(translations.getComponentSync(CommandMessages.TPALL_SUCCESS) {
    ...
    fun tppos(
        ...
        if (target == actor.sender()) {
            actor.sender().sendMessage(translations.getComponentSync(CommandMessages.TPPOS_SUCCESS) {
                unparsed("x", x.toInt().toString())
                unparsed("y", y.toInt().toString())
                unparsed("z", z.toInt().toString())
            })
        } else {
            actor.sender().sendMessage(translations.getComponentSync(CommandMessages.TPPOS_OTHER) {
                unparsed("player", target.name)
                unparsed("x", x.toInt().toString())
                unparsed("y", y.toInt().toString())
                unparsed("z", z.toInt().toString())
    ...
    fun top(actor: BukkitCommandActor) {
        if (actor.sender() !is Player) {
            actor.sender().sendMessage(translations.getComponentSync(CommandMessages.PLAYER_ONLY))
            return
        }
        ...
        actor.sender().sendMessage(translations.getComponentSync(CommandMessages.TOP_SUCCESS))
    }

    @Command("bottom")
    @CommandPermission("essentiallystateless.bottom")
    fun bottom(actor: BukkitCommandActor) {
        if (actor.sender() !is Player) {
            actor.sender().sendMessage(translations.getComponentSync(CommandMessages.PLAYER_ONLY))
            return
        }
        ...
        plugin.launch {
            withContext(plugin.regionDispatcher(player.location)) {
                ...
                if (foundY != null) {
                    ...
                    player.sendMessage(translations.getComponentSync(CommandMessages.BOTTOM_SUCCESS))
                } else {
                    player.sendMessage(translations.getComponentSync(CommandMessages.BOTTOM_FAILED))
                }
            }
        }
    }
    ...
    fun jump(actor: BukkitCommandActor) {
        if (actor.sender() !is Player) {
            actor.sender().sendMessage(translations.getComponentSync(CommandMessages.PLAYER_ONLY))
            return
        }
        ...
        if (targetBlock == null) {
            actor.sender().sendMessage(translations.getComponentSync(CommandMessages.JUMP_FAILED))
            return
        }
        ...
        actor.sender().sendMessage(translations.getComponentSync(CommandMessages.JUMP_SUCCESS))
    }
}
